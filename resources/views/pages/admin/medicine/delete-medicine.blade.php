<div x-data="{open:false}" class="inline">
    <button x-on:click="open = true"
        class="inline-flex items-center gap-1 bg-red-500 hover:bg-red-600 text-white p-1 px-3 text-sm rounded">
        <svg class="h-3 w-3 " viewBox="0 0 16 16" fill="#f5f5f5">
            <path
    ...
    <div x-show="open" x-cloak>
        <div class="absolute top-2/4 left-2/4" style="z-index: 501; transform: translate(-50%, -50%)">
            <div class="p-4 text-left bg-white shadow-lg rounded-lg" style="width: 28rem">
                ...
                <p class="text-xs">
                    This action will permanently delete <span class="font-bold capitalize">{{ $medicine->medicine_name
                        }}</span>.
                ...
                <div class="flex justify-end gap-2 mt-4">
                    ...
                    <form method="POST" action="/medicine/{{ $medicine->id }}">
                        @csrf
                        @method('DELETE')
                        <button type="submit"

<div x-data="{equipmentEditOpen:false}" class="inline">
    <button x-on:click="equipmentEditOpen = true"
    ...
    <div x-show="equipmentEditOpen" x-cloak x-on:click="equipmentEditOpen = false"
        class="bg-black/40 z-[500] fixed top-0 bottom-0 right-0 left-0">
    </div>

    {{-- edit modal --}}
    <div x-show="equipmentEditOpen" x-cloak>
        <div class="absolute bg-slate-50 shadow-md rounded-md top-2/4 left-2/4 w-3/4 -translate-y-2/4 -translate-x-2/4"
            ...
            <header class="flex justify-between items-center  p-4 border rounded-md border-b-1 border-gray-200">
                <h2 class="font-medium">Edit {{ $equipment->equipment_name }}</h2>
                <svg x-on:click="equipmentEditOpen = false" class="h-4 w-4 cursor-pointer" viewBox="0 0 16 16"
                    fill="gray">
                    <path
            ...
            <form class=" p-4" action="/equipment/{{ $equipment->id }}" method="POST">
                @csrf
                @method('PUT')
                <div>
                    ...
                    <x-input id="equipment_name" class="block mt-1 w-full" type="text" name="equipment_name"
                        :value="old('equipment_name')" value="{{$equipment->equipment_name}}" required autofocus />
                </div>
                <div class="mt-4">
                    ...
                    <x-input id="equipment_quantity" class="block mt-1 w-full" type="number" name="equipment_quantity"
                        :value="old('equipment_quantity')" value="{{$equipment->equipment_quantity}}" required
                        autofocus />
                </div>
                <div class="mt-4">
                    ...
                    <x-input id="equipment_cost" class="block mt-1 w-full" type="number" name="equipment_cost"
                        :value="old('equipment_cost')" value="{{$equipment->equipment_cost}}" required autofocus />
                </div>
                <div class="mt-4">
                    <x-label for="equipment_date_of_acquisition" :value="__('Date of Acquisition')" />
                    <x-input id="equipment_date_of_acquisition" class="block mt-1 w-full" type="date"
                        name="equipment_date_of_acquisition" :value="old('equipment_date_of_acquisition')"
                        value="{{$equipment->equipment_date_of_acquisition}}" required autofocus />
                </div>

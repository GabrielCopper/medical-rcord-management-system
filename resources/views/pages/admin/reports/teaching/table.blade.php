{{-- Table --}}
@unless ($teaching_records->isEmpty())
<div class="overflow-x-auto">
    <table class="table-auto w-full">
        ...
        <tbody class="text-sm font-medium divide-y divide-slate-100">
            ...
            @foreach ($teaching_records as $teaching_record)
            <tr>
                <td class="p-2">
                    {{ $teaching_record->user_data->user_patient_first_name }}
                    {{ $teaching_record->user_data->user_patient_last_name }}
                </td>
                <td class="p-2 ">
                    {{ $teaching_record->user_data->user_year_department_role }}
                </td>
                <td class="p-2 ">
                    {{ $teaching_record->complaints }}
                </td>
                <td class="p-2 ">
                    {{ $teaching_record->diagnosis }}
                </td>
                <td class="p-2 ">
                    @foreach(explode('|', $teaching_record->patient_prescribed_medicine) as $medicine)
                    @if ($medicine)
